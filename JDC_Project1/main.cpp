...
// can be changed to change diffuculty
...
int main(int argc, char** argv) {
	...
	while ( !game_over ) {
		...
		while ( (!guessed_code) && (num_guesses < MAX_GUESSES) ) {
			...
			if(player_code_copy == secret_code_copy){
				cout<< "Congratutions, YOu guessed the code!\nIt only took you "<<num_guesses<<" guess(es)\n\n";
				guessed_code= true;
			}
			...
			for (int x = 0; x < CODE_LENGTH; x++){    		
				for(int i = 0; i < CODE_LENGTH; i++){
					if (player_code_copy[x] == secret_code_copy[i]){
					misplaced_digits++;	
					player_code_copy[x]= 'm';
					secret_code_copy[i]='k';
					}
				}
			}	
